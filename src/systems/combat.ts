...
export const KARYUDON_ACTIONS: CombatAction[] = [
  ...
  // --- KING ABILITY (Builds over the fight, dormant/flickering) ---
  {
    ...
    accuracy: 100,
    cooldown: 4, // Powerful but not spammable
    currentCooldown: 0,
    staminaCost: 15,
    ...
    missText: '',
    effects: [
      { type: 'intimidate', value: 20, duration: 3, chance: 100, description: 'Will crushed' },
...
export function calculateDamage(
  ...
  const defenseDebuffs = Math.min(COMBAT.EXPOSE_CAP, target.statusEffects
    .filter((e) => e.type === 'expose')
    .reduce((sum, e) => sum + e.value, 0)); // capped at EXPOSE_CAP DEF to prevent degenerate stacking
